        ...
        scanf("%s", choice, 100);
            ...
            if(strcmp(choice, "a") == 0){
                ...
                */
                ...
                if (connect(TCP, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) == -1){
                    ...
                    write(TCP, &Spdu, sizeof(Spdu));//write 256 bytes from Spdu on TCP connection
                    recv(TCP,&Rpdu, sizeof(Rpdu), 0);
                    printf("%s", Rpdu.Cdata); 
                }
            }
            ...
            else if(strcmp(choice, "b") == 0)
            {
                ...
                scanf("%s", contentName);
                scanf("%s", ipAddr);
                printf("scans: %s %s %s\n", peerName, contentName, ipAddr);
                strcpy(uniqueName, peerName);
                strcpy(Spdu.registerPeer, peerName);
                strcpy(Spdu.registerContent, contentName);
                strcpy(Spdu.registerPort, portNum);
                strcpy(Spdu.registerIP, ipAddr);
                int i;
                ...
                //printf("concat scans: %s %s %s\n", peerName, contentName, ipAddr);
                //strcat(address, portNum);
                ...
                int n = strlen(Spdu.registerPeer) + strlen(Spdu.registerContent) + strlen(Spdu.registerIP) + strlen(Spdu.registerPort);
                int check = write(UDP, &Spdu, n+1);
    ...
    if(FD_ISSET(TCP, &rfds)){//if other peer wants to connect to me...
        printf("\nINCOMING CONNECTION!!\n");
            ...
            if(new_TCP < 0){
                ...
                recv(TCP, &Rpdu, sizeof(Rpdu), 0);//receive file name
